...
一款认知能力自测 App。答 30 道题，得到六个维度的认知画像，并按你的短板生成一份分阶段的读书计划。也可以每天制定日计划、次日复盘执行情况，追踪自己的行动力。
...
题库在 `src/data/questions.ts`，每个维度 5 题，共 30 题。题目分两类：

- **情景判断题**：给出一个具体场景，四个选项体现不同深度的思考方式
- **习惯自评题**：测的是日常行为倾向，而非知识

每个选项带一个 0–100 的分值，不是简单的对错。维度分 = 该维度 5 道题的平均分；总分 = 六个维度的平均分。算分逻辑在 `src/logic/scoring.ts`。
...
1. **今天定计划** — 写下 3–6 件具体可完成的事，可标高/中/低优先级
2. **次日（或当天晚间）复盘** — 每件事标记：完成 / 部分完成 / 未做，可写一句反思
3. **看行动力** — App 根据计划质量与执行率打分，并追踪连续复盘与趋势

算分逻辑在 `src/logic/actionScore.ts`：

- **执行率**：完成=100、部分=50、未做=0，按任务平均
- **计划质量**：任务数量是否克制（3–6 最佳）、标题是否具体、是否区分优先级
- **当日行动力** = 70% 执行率 + 30% 计划质量
- **等级展示**优先用近 7 个已复盘日的平均分

等级：待机（0–34）、点火（35–49）、入轨（50–64）、推进（65–79）、燃速（80+）。
...
│   ├── questions.ts       30 道题库
│   ├── books.ts           45 本书库
│   └── actionHints.ts     行动力分段提示
├── logic/
│   ├── scoring.ts         认知算分引擎
│   ├── recommend.ts       书单推荐算法
│   └── actionScore.ts     行动力算分与洞察
...
└── screens/
    ...
    ├── ActionScreen.tsx   行动力总览
    ├── ActionPlanScreen.tsx   日计划编辑
    └── ActionReviewScreen.tsx 执行复盘
scripts/verify.ts          数据与算法自检
```

## 怎么改内容

**加题**：在 `src/data/questions.ts` 里加一条，指定 `dimension`，给每个选项标 0–100 的分值。注意 `scripts/verify.ts` 会校验每个维度是 5 题，加题时同步改一下那条断言。
